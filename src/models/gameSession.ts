import { DatedItem, IdItem, Reference } from '.'
...
export type SessionPrepIndex = Reference & DatedItem & SessionPrepBase
...
export function getSessionPrepIndex (session: SessionPrep): SessionPrepIndex {
  ...
  const { title, id, synopsis, previousSessionId, created, lastEdited } = session
  return { title, id, synopsis, previousSessionId, created, lastEdited }
}

export type SessionPrep = IdItem & DatedItem &
  SessionPrepBase & {
    userId: string
...
export type RunSection = IdItem &
  (PlayerCharacterPrep | ListPrep | ListRun | TextPrep | ChapterPrep | ImagePrep | LinkPrep)

export type PrepSection = IdItem & (PlayerCharacterPrep | ListPrep | TextPrep | ChapterPrep | ImagePrep | LinkPrep)
